...
| Evaluation & plotting boilerplate | [src/evaluation.py](src/evaluation.py), [src/data_visualization.py](src/data_visualization.py) | Matplotlib styling, SHAP plot wrappers, and the reliability / calibration plot skeleton drafted with AI help. Backtest logic, baselines, and ablation design are ours. |
| GroupKFold CV scaffolding | [src/models.py](src/models.py) (`hyperparam_search_cv`) | Structure of the fold loop and per-fold scaler placement debugged with AI assistance after we identified a CV leakage bug. Hyperparameter grid and selection criterion are ours. |
...
**None.** Both the LightGBM model ([kalshi_lgbm.txt](kalshi_lgbm.txt)) and the XGBoost model ([kalshi_xgb.json](kalshi_xgb.json)) were trained from scratch on the Kalshi candle data. The scaler ([kalshi_scaler.pkl](kalshi_scaler.pkl)) is fit on training data only. No transfer learning or pretrained weights are used.
